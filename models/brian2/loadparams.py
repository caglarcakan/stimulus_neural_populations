import numpy as np
from utils import generate_OUinput, x_filter, get_changing_input, interpolate_input
...
def set_network_params(params, stimulus = []):
    ...
    net_params['net_record_all_neurons'] = True
    
    net_params['delay_type'] = 1
    net_params['const_delay_e'] = params['de']
    net_params['const_delay_i'] = params['di']

    net_params['connectivity_type'] = 'fixed'
    
    net_params['a'] = params['a'] 
    net_params['b'] = params['b']

    net_params['N_e'] = params['N'] 
    net_params['N_i'] = params['N'] 

    net_params['K_ee'] = int(params['Ke'])
    net_params['K_ii'] = int(params['Ki'])
    net_params['K_ei'] = int(params['Ki'])
    net_params['K_ie'] = int(params['Ke'])
        
    net_params['cee'] = params['cee']
    net_params['cie'] = params['cie'] 
    net_params['cii'] = params['cii'] 
    net_params['cei'] = params['cei']

    net_params['J_ee'] = params['Jee_max']
    net_params['J_ii'] = params['Jii_max']
    net_params['J_ei'] = params['Jei_max']
    net_params['J_ie'] = params['Jie_max']

    mu_e_ext = params['mue_ext_mean']
    sigma_e_ext = params['sigmae_ext']
    mu_i_ext = params['mui_ext_mean']
    sigma_i_ext = params['sigmai_ext']

    steps = int(net_params['runtime']/net_params['net_dt'])
    t_ext = np.linspace(0., net_params['runtime'], steps+1)
    net_params['t_ext'] = t_ext
    net_params['min_dt'] = net_params['net_dt']

    mu_ext = np.ones(steps+1) * mu_e_ext
    sigma_ext = np.ones(steps+1) * sigma_e_ext

    if len(stimulus) > 0:
        mu_ext = np.ones(len(stimulus)) * mu_e_ext + stimulus
        
    ext_input0 = [mu_ext, sigma_ext]
    ext_input_e = interpolate_input(ext_input0,net_params,'net')

    ext_input_i_raw_mean = np.ones(steps+1) * mu_i_ext
    ext_input_i_raw_sigma = np.ones(steps+1) * sigma_i_ext
    ext_input_i = interpolate_input([ext_input_i_raw_mean,ext_input_i_raw_sigma], net_params,'net')

    ext_input = [ext_input_e, ext_input_i]
    
    net_params['net_record_spikes'] = min(100, net_params['N_e'])
    return net_params, ext_input

def init_resultsdict():
    results = dict()
