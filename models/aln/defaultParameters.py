...
def loadDefaultParams(CmatFileName = "", Cmat = [], DmatFileName = "", Dmat = [], signalV = 7.5, singleNode = 0, lookupTableFileName = None, seed=None):
    ...
    params.Ke_gl        = 100.   # number of incoming E connections (to E population) from each area
    ...
    params.Irange = hf.get('mu_vals').value
    params.sigmarange = hf.get('sigma_vals').value
    params.dI = params.Irange[1] - params.Irange[0]
    params.ds = params.sigmarange[1] - params.sigmarange[0]

    params.precalc_r = hf.get('r_ss').value
    params.precalc_V = hf.get('V_mean_ss').value
    params.precalc_tau_mu = hf.get('tau_mu_exp').value
    params.precalc_tau_sigma = hf.get('tau_sigma_exp').value
